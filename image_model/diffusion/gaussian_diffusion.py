...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, condition, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        _,x_output = model(condition, t, x, **model_kwargs)
        # breakpoint()
        if isinstance(x_output, tuple):
            x_output, extra = x_output
        else:
            extra = None

        self.model_var_type = ModelVarType.FIXED_SMALL
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            assert x_output.shape == (B, C * 2, *x.shape[2:])
            x_output, model_var_values = th.split(x_output, C, dim=1)
        ...
        if self.model_mean_type == ModelMeanType.START_X:
            pred_xstart = process_xstart(x_output)
            # pred_timeembstart = process_xstart(time_emb_output)
        else:
            pred_xstart = process_xstart(
                self._predict_xstart_from_eps(x_t=x, t=t, eps=x_output)
            )
            # pred_timeembstart = process_xstart(
            #     self._predict_xstart_from_eps(x_t=time_emb, t=t, eps=time_emb_output)
            # )
        x_mean, _, _ = self.q_posterior_mean_variance(x_start=pred_xstart, x_t=x, t=t)
        # time_emb_mean, _, _ = self.q_posterior_mean_variance(x_start=pred_xstart, x_t=x, t=t)


        assert x_mean.shape == model_log_variance.shape == pred_xstart.shape == x.shape
        return {
            "mean": x_mean,
            # "time_emb_mean":time_emb_mean,
            "variance": model_variance,
            "log_variance": model_log_variance,
            "pred_xstart": pred_xstart,
            # "pred_timeembstart":pred_timeembstart,
            "extra": extra,
        }
    ...
    def _vb_terms_bpd(
            self, model, x_start, x_t, time_emb_start, time_emb_t, t, clip_denoised=True, model_kwargs=None
    ):
        """
        ...
        """
        true_x_mean, _, true_x_log_variance_clipped = self.q_posterior_mean_variance(
            x_start=x_start, x_t=x_t, t=t
        )
        true_t_mean, _, true_t_log_variance_clipped = self.q_posterior_mean_variance(
            x_start=time_emb_start, x_t=time_emb_t, t=t
        )
        out = self.p_mean_variance(
            model, x_t, t, time_emb_t, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )
        kl = normal_kl(
            true_x_mean, true_x_log_variance_clipped, out["x_mean"], out["x_log_variance"]
        )
        kl = mean_flat(kl) / np.log(2.0)

        decoder_nll = -discretized_gaussian_log_likelihood(
            x_start, means=out["x_mean"], log_scales=0.5 * out["x_log_variance"]
        )
    ...
    def training_losses(self, model, x_start, t, time_emb_start, model_kwargs=None, noise=None, block_size=96, patch_size=16, add_mask=False):
        ...
        plt.imsave('t_t.png',time_emb_start[0].cpu().numpy())

        x_t = self.q_sample(x_start, t, noise=noise_x)
        time_emb_t = self.q_sample(time_emb_start, t, noise=noise_time_emb)
        x_t = x_t * (1-masks) + masks * x_start
        terms = {}
